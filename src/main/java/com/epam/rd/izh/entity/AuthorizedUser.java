 ...
 * быть использована как часть бизнес логики приложения, например сотрудник больницы, где role определяет его
 * полномочия.
 *
 * Рекомендуется добавить поле id в сущность пользователя. Поле id может генерироваться базой данных, также можно
...
public class AuthorizedUser {
  ...
  /**
   * Определяет GrantedAuthority пользователя. Может быть коллекцией, например Set<Strings> если логика приложения
